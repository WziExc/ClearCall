...
> **最后更新**：2026-06-09（自建 WebSocket 中继 + 扫码 SDP 交换 — 国内双方案）  
...
### 双项目分支规则 🔴
> `clearcall/`（主开发线）、`clearcall-classic/`（经典版）、`clearcall-advanced/`（高级版）是同一项目的三个分支。

- 用户发起修改请求时，**如果没有明确指出目标版本，必须先询问确认**，不得擅自选择版本。
- 三个分支的代码应保持独立修改，不可混用。
- 修改后需分别在对应分支上构建验证。
- 详见 [09-dev-governance.md §1.4](clearcall-dev/09-dev-governance.md)
